...
describe('intersect', () => {
  ...
  it('should work with all primitive types', () => {
    ...
    expect(Arr.intersect([obj, foo, bar, num, str], [obj, num])).toEqual([
      obj,
      num,
    ])
    expect(Arr.intersect([obj, foo, num, str], [obj, bar, str])).toEqual([
      obj,
      str,
    ])
    expect(Arr.intersect([bar, foo], [obj, foo, bar, num, str])).toEqual([
      bar,
      foo,
    ])
  })
